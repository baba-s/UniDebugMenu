...
namespace Kogane.Internal
{
	...
	internal sealed class CommandListUI : DebugMenuUIBase
	{
		//==============================================================================
		// クラス
		//==============================================================================
		[Serializable]
		public sealed class LoopListViewUI : LoopListViewUI<CommandData, CommandListElemUI>
		{
		}
		...
		protected override void DoSetDisp( object creator )
		{
			...
			m_view.mOnComplete = ( elemData, elemIndex ) =>
			{
				...
				{
					var isOn   = elemData.ToggleActionData.Getter();
					var result = isOn ? "オン" : "オフ";
					OpenToastUI( $"[{text}] [{result}] 完了" );
				}
				else
				{
					OpenToastUI( $"[{text}] [{elemData.ActionDataList[ elemIndex - 2 ].Text}] 完了" );
				}
			};
		}
